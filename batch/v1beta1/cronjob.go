...
func CreateOrPatchCronJob(c kubernetes.Interface, meta metav1.ObjectMeta, transform func(*batch.CronJob) *batch.CronJob) (*batch.CronJob, error) {
	...
	if kerr.IsNotFound(err) {
		...
		return c.BatchV1beta1().CronJobs(meta.Namespace).Create(transform(&batch.CronJob{
...
func PatchCronJob(c kubernetes.Interface, cur *batch.CronJob, transform func(*batch.CronJob) *batch.CronJob) (*batch.CronJob, error) {
...
func TryPatchCronJob(c kubernetes.Interface, meta metav1.ObjectMeta, transform func(*batch.CronJob) *batch.CronJob) (result *batch.CronJob, err error) {
	...
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		if kerr.IsNotFound(e2) {
			...
			result, e2 = PatchCronJob(c, cur, transform)
			return e2 == nil, nil
		}
