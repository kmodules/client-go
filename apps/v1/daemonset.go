...
func CreateOrPatchDaemonSet(ctx context.Context, c kubernetes.Interface, meta metav1.ObjectMeta, transform func(*apps.DaemonSet) *apps.DaemonSet) (*apps.DaemonSet, kutil.VerbType, error) {
	...
	return PatchDaemonSet(ctx, c, cur, transform)
}

func PatchDaemonSet(ctx context.Context, c kubernetes.Interface, cur *apps.DaemonSet, transform func(*apps.DaemonSet) *apps.DaemonSet) (*apps.DaemonSet, kutil.VerbType, error) {
	return PatchDaemonSetObject(ctx, c, cur, transform(cur.DeepCopy()))
}

func PatchDaemonSetObject(ctx context.Context, c kubernetes.Interface, cur, mod *apps.DaemonSet) (*apps.DaemonSet, kutil.VerbType, error) {
	...
	out, err := c.AppsV1().DaemonSets(cur.Namespace).Patch(ctx, cur.Name, types.StrategicMergePatchType, patch, metav1.PatchOptions{})
	return out, kutil.VerbPatched, err
}

func TryUpdateDaemonSet(ctx context.Context, c kubernetes.Interface, meta metav1.ObjectMeta, transform func(*apps.DaemonSet) *apps.DaemonSet) (result *apps.DaemonSet, err error) {
	attempt := 0
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		if kerr.IsNotFound(e2) {
			...
			result, e2 = c.AppsV1().DaemonSets(cur.Namespace).Update(ctx, transform(cur.DeepCopy()), metav1.UpdateOptions{})
			return e2 == nil, nil
		}
