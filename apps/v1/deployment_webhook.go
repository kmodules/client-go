...
func (a *DeploymentWebhook) Admit(req *admission.AdmissionRequest) *admission.AdmissionResponse {
	...
	case admission.Delete:
		...
		obj, err := a.client.AppsV1().Deployments(req.Namespace).Get(req.Name, metav1.GetOptions{})
	...
	case admission.Create:
		v1Obj, originalObj, err := convert_to_v1_deployment(gv, req.Object.Raw)
		if err != nil {
			return api.StatusBadRequest(err)
		}
	...
	case admission.Update:
		...
		v1OldObj, _, err := convert_to_v1_deployment(gv, req.OldObject.Raw)
		if err != nil {
			return api.StatusBadRequest(err)
		}
...
func convert_to_v1_deployment(gv schema.GroupVersion, raw []byte) (*v1.Deployment, runtime.Object, error) {
	switch gv {
...
func create_deployment_patch(gv schema.GroupVersion, originalObj, v1Mod interface{}) ([]byte, error) {
	internalObj := &extensions.Deployment{}
	...
	case v1beta1.SchemeGroupVersion:
		v1beta1Mod := &v1beta1.Deployment{}
		err := legacyscheme.Scheme.Convert(internalObj, v1beta1Mod, nil)
		if err != nil {
			return nil, err
		}
		legacyscheme.Scheme.Default(v1beta1Mod)
