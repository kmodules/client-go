...
import (
	...
	nocache "kmodules.xyz/client-go/tools/cache"
...
func (f *dynamicSharedInformerFactory) ForResource(gvr schema.GroupVersionResource) cache.SharedInformer {
	...
	informer = NewFilteredDynamicInformer(f.client, gvr, f.namespace, f.defaultResync, f.tweakListOptions)
...
func NewFilteredDynamicInformer(client dynamic.Interface, gvr schema.GroupVersionResource, namespace string, resyncPeriod time.Duration, tweakListOptions TweakListOptionsFunc) cache.SharedInformer {
	return nocache.NewSharedInformer(
		&cache.ListWatch{
