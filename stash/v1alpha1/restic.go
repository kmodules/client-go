...
func PatchRestic(c tcs.StashV1alpha1Interface, cur *aci.Restic, transform func(*aci.Restic) *aci.Restic) (*aci.Restic, error) {
	...
	patch, err := strategicpatch.CreateTwoWayMergePatch(curJson, modJson, aci.Restic{})
	if err != nil {
		return nil, err
	}
	...
	result, err := c.Restics(cur.Namespace).Patch(cur.Name, types.StrategicMergePatchType, patch)
	return result, err
}
