...
func UpdateDeploymentExtension(c clientset.Interface, meta metav1.ObjectMeta, transformer func(extensions.Deployment) extensions.Deployment) error {
	attempt := 0
	for ; attempt < maxAttempts; attempt = attempt + 1 {
		...
		if kerr.IsNotFound(err) {
			...
			_, err = c.ExtensionsV1beta1().Deployments(cur.Namespace).Update(&modified)
			if err == nil {
				return nil
			}
		}
