...
func PatchCertificate(c tcs.VoyagerV1beta1Interface, cur *aci.Certificate, transform func(*aci.Certificate) *aci.Certificate) (*aci.Certificate, error) {
	...
	patch, err := strategicpatch.CreateTwoWayMergePatch(curJson, modJson, aci.Certificate{})
	if err != nil {
		return nil, err
	}
	...
	result, err := c.Certificates(cur.Namespace).Patch(cur.Name, types.StrategicMergePatchType, patch)
	return result, err
}
