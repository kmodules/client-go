...
func (d *Doctor) findMasterPodsByLabel() ([]core.Pod, error) {
	pods, err := d.kc.CoreV1().Pods(metav1.NamespaceSystem).List(context.TODO(), metav1.ListOptions{
...
func (d *Doctor) findMasterPodsByKubernetesService() ([]core.Pod, error) {
	ep, err := d.kc.CoreV1().Endpoints(core.NamespaceDefault).Get(context.TODO(), "kubernetes", metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	...
	lister := pager.New(func(ctx context.Context, opts metav1.ListOptions) (runtime.Object, error) {
		return d.kc.CoreV1().Pods(metav1.NamespaceSystem).List(context.TODO(), opts)
	})
	objects, _, err := lister.List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
